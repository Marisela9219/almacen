...
class Umilitar extends CI_Controller {
	...
	public function store(){
		...
		if ($this->form_validation->run()) {
			...
			if ($this->Clientes_model->save($data)) {
				redirect(base_url()."mantenimiento/umilitar");
			}
			else{
				$this->session->set_flashdata("error","No se pudo guardar la informacion");
				redirect(base_url()."mantenimiento/umilitar/add");
			}
		}
		else{
			$this->add();
	...
	public function update(){
		...
		if ($this->form_validation->run()) {
			...
			if ($this->Clientes_model->update($idcliente,$data)) {
				...
				redirect(base_url()."mantenimiento/umilitar/edit/".$idcliente);
			}
		}else{
			$this->edit($idcliente);
		}
